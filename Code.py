...
def accept_and_validate_file():
    ...
    file_path = filedialog.askopenfilename(
        title="Select an Office file",
        filetypes=[("Office files", "*.xls *.xlsm *.xlsb *.xlsx *.docx *.docm *.pptx *.pptm")],  # Include Word and PowerPoint files
        initialdir="/"  # Optionally set an initial directory
    )

    # Validate the chosen file
    valid_extensions = ('.xls', '.xlsm', '.xlsb', '.xlsx', '.docx', '.docm', '.pptx', '.pptm')
    if file_path and file_path.lower().endswith(valid_extensions):
        return file_path
    else:
        print("Invalid file type selected. Please select a valid Office file.")
        return None
...
def extract_vba(file_path):
    ...
    if file_path.endswith(('.xls', '.xlsm', '.xlsb', '.docm', '.pptm')):
...
# Detection functions for different types of malicious activities
...
def check_network_functions(vba_code):
    patterns = ["Winsock", "HTTP", "SendKeys"]
    risk_score = 0
    for pattern in patterns:
        if re.search(re.escape(pattern), vba_code, re.IGNORECASE):
...
def check_environment_functions(vba_code):
    ...
    for pattern in patterns:
        if re.search(pattern, vba_code):
            risk_score = update_risk_score(risk_score, 15, f"Accessing environment variables or system information using {pattern}")
    return risk_score
...
# Main parsing function that invokes all checks
def parse_vba_content(vba_content):
    ...
    if readable_text:
        ...
        if risk_percentage == 0:
            print("The file appears to be safe, with no malicious indicators detected.")
        elif risk_percentage <= 25:
            print("Low risk detected. Be cautious of minor potential threats in the file.")
        elif risk_percentage <= 50:
            print("Moderate risk detected. This file contains elements that could be potentially harmful.")
        elif risk_percentage <= 75:
            print("High risk detected. This file likely contains malicious components.")
        else:
            print("Very high risk detected. This file is almost certainly harmful and should not be trusted.")

        print(f"Total risk score: {total_risk_score} out of {max_possible_score} ({risk_percentage:.2f}%)")


def main():
    file_path = accept_and_validate_file()
    if file_path:
